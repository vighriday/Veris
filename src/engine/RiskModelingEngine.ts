...
import { DiffReport, RiskReport, RiskScore } from '../models/RiskModels';
...
export class RiskModelingEngine {
    ...
    public assessRisk(diff: DiffReport, currentGraph: BehavioralGraph): RiskReport[] {
        ...
        diff.impactedNodes.forEach(node => {
            ...
            if (blastRadius > 50) explanations.push(`High blast radius due to ${integrationCount} integrations.`);
            if (runtimeCriticality > 80) explanations.push(`High runtime criticality assigned (core engine or auth path).`);
            if (dependencyFragility > 50) explanations.push(`High dependency fragility due to dense coupling.`);
